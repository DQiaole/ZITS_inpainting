...
class HourglassNet(nn.Module):
    ...
    def __init__(self, input_channel, inplanes, num_feats, block, head, depth, num_stacks, num_blocks, num_classes):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        # if the inputsize is 256，remain the hourglass input/output is 128
        if x.shape[2] >= 256:
            x = self.maxpool(x)
        x = self.layer2(x)
        x = self.layer3(x)
